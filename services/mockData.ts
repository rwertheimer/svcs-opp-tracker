...
export const generateAccountDetails = (accountId: string): AccountDetails => {
    const supportTickets: SupportTicket[] = Array.from({ length: 3 }, (_, i) => ({
    ...
    const numIntegrations = Math.floor(Math.random() * 3) + 2; // 2 to 4 integrations
    ...
    for(const integration of selectedIntegrations) {
        let lastMonthBillable = Math.random() * 1e9 + 1e7; // Start with a baseline
        for (let i = 0; i < 3; i++) { // For the last 3 months
            const date = new Date();
            date.setMonth(date.getMonth() - i);
            
            // Fluctuate volume month to month
            const fluctuation = (Math.random() - 0.4); // Can go up or down
            const billable = Math.max(0, lastMonthBillable * (1 + fluctuation));
            const raw = billable * (Math.random() * 2 + 1.2); // Raw is always more
    ...
    const projectHistory: ProjectHistory[] = Array.from({ length: 2 }, (_, i) => ({
        accounts_salesforce_account_id: accountId,
        accounts_outreach_account_link: 'http://example.com',
        accounts_salesforce_account_name: 'Mock Account',
